...
Read `user_language` from `~/.claude/exovibe/config.json`. Render any
conversational output (mode-selection prompts, success confirmations, "opened
in browser" messages, dependency-missing guidance) in that language. The
dashboard HTML itself is generated by a separate script and uses English
chrome in v0.4.x — that will migrate in a future release.
